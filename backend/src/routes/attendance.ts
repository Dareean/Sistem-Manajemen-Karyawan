...
const computeWorkHours = (checkIn?: Date | null, checkOut?: Date | null) => {
...
router.get("/", async (req, res, next) => {
  try {
    ...
    const filters: Prisma.AttendanceLogWhereInput = {};
...
router.put("/:id", async (req, res, next) => {
  try {
    ...
    const parsed = attendanceSchema.partial().parse(req.body);
    const checkInValue =
